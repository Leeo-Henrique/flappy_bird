import pygame
import os
import random

TELA_LARGURA = 500
TELA_ALTURA = 800

IMAGE_CHAO = pygame.transform.scale2x(
    pygame.image.load(
        os.path.join(
            "imgs_game",
            "base.png",
        )
    )
)

IMAGE_BACKGROUND = pygame.transform.scale2x(
    pygame.image.load(
        os.path.join(
            "imgs_game",
            "bg.png",
        )
    )
)

IMAGE_CANO = pygame.transform.scale2x(
    pygame.image.load(
        os.path.join(
            "imgs_game",
            "pipe.png",
        )
    )
)

IMAGE_PASSARO = [
    pygame.transform.scale2x(
        pygame.image.load(
            os.path.join(
                "imgs_game",
                "bird1.png",
            )
        )
    ),
    pygame.transform.scale2x(
        pygame.image.load(
            os.path.join(
                "imgs_game",
                "bird2.png",
            )
        )
    ),
    pygame.transform.scale2x(
        pygame.image.load(
            os.path.join(
                "imgs_game",
                "bird3.png",
            )
        )
    ),
]

pygame.font.init()
FONTE_DOS_PONTOS = pygame.font.SysFont("arial", 50)
bigfont = pygame.font.Font(None, 80)
font = pygame.font.Font(None, 45)

import pygame


class Passaro:
    IMGS = IMAGE_PASSARO
    ROTACAO_MAXIMA = 25
    VELOCIDADE_ROTACAO = 20
    TEMPO_ANIMACAO = 5

    def __init__(self, x, y) -> None:
        self.x = x
        self.y = y

        self.angulo = 0
        self.velocidade = 0
        self.tempo = 0
        self.altura = self.y
        self.contagem_img = 0
        self.img = self.IMGS[0]

    def pular(self):
        self.velocidade = -10.5
        self.tempo = 0
        self.altura = self.y

    def mover(self):
        self.tempo += 1
        deslocamento = 1.5 * (self.tempo**2) + self.velocidade * self.tempo

        if deslocamento > 16:
            deslocamento = 16
        elif deslocamento < 0:
            deslocamento -= 2

        self.y += deslocamento

        if deslocamento < 0 or self.y < (self.altura + 50):
            if self.angulo < self.ROTACAO_MAXIMA:
                self.angulo = self.ROTACAO_MAXIMA
        else:
            if self.angulo > -90:
                self.angulo -= self.VELOCIDADE_ROTACAO

    def desenhar(self, tela):
        # definir qual imagem o passaro vai usar
        self.contagem_img += 1

        if self.contagem_img < self.TEMPO_ANIMACAO:
            self.img = self.IMGS[0]
        if self.contagem_img < self.TEMPO_ANIMACAO * 2:
            self.img = self.IMGS[1]
        if self.contagem_img < self.TEMPO_ANIMACAO * 3:
            self.img = self.IMGS[2]
        if self.contagem_img < self.TEMPO_ANIMACAO * 4:
            self.img = self.IMGS[1]
        if self.contagem_img < self.TEMPO_ANIMACAO * 4 + 1:
            self.img = self.IMGS[0]
            self.contagem_img = 0

        # se o passaro estiver caindo nao bater a asa

        if self.angulo <= -80:
            self.img = self.IMGS[1]
            self.contagem_img = self.TEMPO_ANIMACAO * 2

        # desenhar passaro

        imagem_rotacionada = pygame.transform.rotate(self.img, self.angulo)
        pos_centro_img = self.img.get_rect(topleft=(self.x, self.y)).center
        retangulo = imagem_rotacionada.get_rect(center=pos_centro_img)
        tela.blit(imagem_rotacionada, retangulo.topleft)

    def get_mask(self):
        return pygame.mask.from_surface(self.img)


class Cano:
    DISTANCIA = 200
    VELOCIDADE = 5

    def __init__(self, x) -> None:
        self.x = x
        self.altura = 0
        self.pos_topo = 0
        self.pos_base = 0
        self.cano_topo = pygame.transform.flip(IMAGE_CANO, False, True)
        self.cano_base = IMAGE_CANO

        self.passou = False
        self.definir_altura()

    def definir_altura(self):
        self.altura = random.randrange(50, 450)
        self.pos_topo = self.altura - self.cano_topo.get_height()
        self.pos_base = self.altura + self.DISTANCIA

    def mover(self):
        self.x -= self.VELOCIDADE

    def desenhar(self, tela):
        tela.blit(self.cano_topo, (self.x, self.pos_topo))
        tela.blit(self.cano_base, (self.x, self.pos_base))

    def colidir(self, passaro):
        passaro_mask = passaro.get_mask()
        topo_mask = pygame.mask.from_surface(self.cano_topo)
        base_mask = pygame.mask.from_surface(self.cano_base)

        distancia_topo = (self.x - passaro.x, self.pos_topo - round(passaro.y))
        distancia_base = (self.x - passaro.x, self.pos_base - round(passaro.y))

        topo_colisao = passaro_mask.overlap(topo_mask, distancia_topo)
        base_colisao = passaro_mask.overlap(base_mask, distancia_base)

        if base_colisao or topo_colisao:
            return True
        else:
            return False


class Chao:
    VELOCIDADE = 5
    LARGURA = IMAGE_CHAO.get_width()
    IMAGEM = IMAGE_CHAO

    def __init__(self, y):
        self.y = y
        self.x1 = 0
        self.x2 = self.LARGURA

    def mover(self):
        self.x1 -= self.VELOCIDADE
        self.x2 -= self.VELOCIDADE

        if self.x1 + self.LARGURA < 0:
            self.x1 = self.x2 + self.LARGURA
        if self.x2 + self.LARGURA < 0:
            self.x2 = self.x1 + self.LARGURA

    def desenhar(self, tela):
        tela.blit(self.IMAGEM, (self.x1, self.y))
        tela.blit(self.IMAGEM, (self.x2, self.y))


def desenhar_tela(tela, passaros, canos, chao, score):
    tela.blit(IMAGE_BACKGROUND, (0, 0))

    for passaro in passaros:
        passaro.desenhar(tela)

    for cano in canos:
        cano.desenhar(tela)

    message = FONTE_DOS_PONTOS.render(f"Pontuação: {score}", 1, (255, 255, 255))
    tela.blit(message, (TELA_LARGURA - 10 - message.get_width(), 10))

    chao.desenhar(tela)
    pygame.display.update()


def game():
    passaros = [Passaro(230, 350)]

    chao = Chao(730)
    canos = [Cano(700)]
    tela = pygame.display.set_mode((TELA_LARGURA, TELA_ALTURA))
    score = 0
    relogio = pygame.time.Clock()

    game_active = True

    while game_active:
        relogio.tick(30)

        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                game_active = False
                pygame.quit()
                quit()
            if event.type == pygame.KEYDOWN and game_active:
                if event.key == pygame.K_SPACE:
                    for passaro in passaros:
                        passaro.pular()
            if event.type == pygame.KEYDOWN and game_active is False:
                if event.key == pygame.K_SPACE:
                    game_active = True

        for passaro in passaros:
            passaro.mover()
        chao.mover()

        adicionar_cano = False
        remover_canos = []

        for cano in canos:
            for i, passaro in enumerate(passaros):
                if cano.colidir(passaro):
                    passaros.pop(i)
                    game_active = False
                if not cano.passou and passaro.x > cano.x:
                    cano.passou = True
                    adicionar_cano = True
            cano.mover()
            if cano.x + cano.cano_topo.get_width() < 0:
                remover_canos.append(cano)

        if adicionar_cano:
            score += 1
            canos.append(Cano(600))

        for cano in remover_canos:
            canos.remove(cano)

        for i, passaro in enumerate(passaros):
            if (passaro.y + passaro.img.get_width()) > chao.y or passaro.y < 0:
                passaros.pop(i)
                game_active = False

        desenhar_tela(tela, passaros, canos, chao, score)


game()
